...
import { dateToString, getDatesBetween, isHoraDentroDeRango } from "../../utilities/funcsDates/funcs";
import { CumplimientosXcred, DiaCal, Periodo, Res_RolHeaders, RutaCumplimiento, TarjetaData } from "../interfaces/getCumplimiento.interfaces";
...
class CumplimientoRol {
    ...
    evJornada(jornadaPlaneada: DiaCal, dia: string):[EvJornada, any] {
        ...
        // Si es turno 1 siempre sale del modulo, se debe checar con el cc al que llego 
        const jReal_lug_ini_cc =  turno==1 ? jReal?.data[0].cc_llg : jReal?.data[0].cc_sld
        ...
        const ev_lug_ini_cc: [boolean, [any,any]] =              this.evCC(     lug_ini_cc, jReal_lug_ini_cc);
    ...
    evRuta(att: AttributesEvRutaModalidad): [boolean, [any, any]] {
        const {dia, eco, cred, isDiaDescanso} = att
        ...
        if( isDiaDescanso && real_rutas_unicos.length>0 ) return [false, ['Descanso', formatoOpcionesY.format(real_rutas_unicos)]]
        else if( isDiaDescanso && real_rutas_unicos.length==0 ) return [true, ['Descanso', '---']]
    ...
    evModalidad(att: AttributesEvRutaModalidad): [boolean, [any, any]] {
        ...
        // Si es descanso no debe salir
        if( isDiaDescanso && real_modalidades_unicos.length>0 ) return [false, ['Descanso', formatoOpcionesY.format(real_modalidades_unicos)]]
        else if( isDiaDescanso && real_modalidades_unicos.length==0 ) return [true, ['Descanso', '---']]
        ...
        if( 
            ...
            (real_modalidades_unicos.length==1 && plan_ruta && real_modalidades_unicos[0]==plan_ruta[0]) 
        ) {
        ...
        return [false, [plan_ruta || 'Ordinario', formatoOpcionesY.format(real_modalidades_unicos)]]
    }

    getCumplimientoOperador_jornada(jornada: DiaCal) {
        const { op_cred, turno, dia, op_estado } = jornada;
        const [
    ...
    getCumplimientoXecos(ecos: number[]): RutaCumplimiento{
        ...
        const cumplimientoDeEcos = ecos.map( eco => {
            ...
            const cumplimientosXdia = fechas.map( dia => {
                ...
                const plan_descansos = plan_all.filter( rol => rol.op_estado != 'L' )
                const turnosLaborando = Math.abs(plan_all.length - plan_descansos.length)     // La diferencia es para saber si todos los turnos descansan
                const isDiaDescanso = (turnosLaborando==0) ? true : false

                let cuentaTotal = 0;
                let cuentaOk = 0;

                const ev_ruta = this.evRuta({dia, eco, isDiaDescanso})
                const ev_modalidad = this.evModalidad({dia, eco, isDiaDescanso})
                const cumplimiento = {
                    ruta:       ev_ruta[0],
                ...
                if(cumplimientosXcred.length>0){
                    ...
                    if(turnosLaborando == 0) {
                        plan_descansos.filter( rol => rol.op_tipo == 'ORD' ).map( (obj, i) => {
                ...
                return { eco, servicio: plan_all[0]?.servicio||'_*¹', fecha: dia, eco_cumplimiento, cumplimientosXcred, cumplimiento, cumplimientoDesc }
            })
