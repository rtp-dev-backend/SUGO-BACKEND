...
import { view_pv } from "../Models/pv_eco.model"
// import { pveconomico } from "../Models/pv_eco.model"
...
export const getEstados = async (req: Request, res: Response) => {
    ...
    const include: Includeable[] = [ 
        ...
        { model: view_pv, attributes: ['eco', 'modulo', 'estatus'],
            where: {
                [Op.and]: [
                    { modulo: { [Op.ne]: 66 } },
                    { estatus: { [Op.ne]: 8 } }
    ...
    if(modulo_puerta && modulo_puerta != '0') where.modulo_puerta = modulo_puerta;
    ...
    else if((estatus as any)=='all') undefined;
...
export const viewPVController = async (req: Request, res: Response) => {
    const { modulo } = req.body;
    try {
        // Consulta usando el ORM de Sequelize
        const resultados = await view_pv.findAll({
            where: { modulo }
        });
